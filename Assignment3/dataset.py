...
class Dictionary(object):
    def __init__(self):
        self.word2idx = {'<s>':0,'</s>':1} #consisting of (word, id) pairs
        self.idx2word = ['<s>','</s>'] #list containing all the words encountered
        self.tag2idx = {}
        self.idx2tag = []
        ...
        with open(path, 'r', encoding="utf8") as f:
            ...
            for line in f:
                ...
                if line:
                    ...
                    seq = torch.zeros(len(ids), 2 * self.w + 1, dtype=torch.int64)
                    ids = [self.dictionary.word2idx['<s>']] * self.w + ids + [self.dictionary.word2idx['</s>']] * self.w
                    for i in range(self.w, len(ids) - self.w):
                        seq[i - self.w,] = torch.tensor(ids[i - self.w:i + self.w + 1], dtype=torch.int64)
#                    print(seq)
                    data.append((seq, torch.tensor(labs).type(torch.int64)))
                    ids = []
                    labs = []
        return data
